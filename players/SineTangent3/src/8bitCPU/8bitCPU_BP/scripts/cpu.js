 ...
 */
function binary(num) {
  if (num < -128 || num > 127) return "error";
 ...
 * @returns { number }
 */
function alu(binary1, binary2, mode) {
  if (binary1 === `error` || binary2 === `error`) return -1;
  let result = ``;
  switch (mode) {
    case 0:
    ...
    onTick(ev) {
      ...
      if (x == Number(listm[0]) && y == Number(listm[1]) && z == Number(listm[2]) && dimId == listm[3]) {
        ...
        if (head == 0) {
          ...
          const binary1 = binary(lists[1]);
          const binary2 = binary(lists[1]);
          if (outR <= 3) {
            lists[3] = alu(binary1, binary2, outR);
          } else {
            switch (outR) {
              case 4:
          ...
          if (outR >= 6) {
            const players = dim.getPlayers({ location: loc, maxDistance: 30 });
            players.map(m => m.sendMessage(`Reg${inR}: ${lists[inR]}`));
          } else {
